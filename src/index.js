...
function defaultBeforeConvert({ resourcePath, resourceQueryObj, outputPath }) {
...
function WebPPreferLoader(content) {
  ...
  const webpOptionStr = options.convertOption || "-q 90";
    ...
    if (webpSrcPath || webpOutputPath) {
      ...
      if (webpSrcPath && fs.existsSync(webpSrcPath)) {
        ...
        webpContentPromise = webp
          .buffer2webp(content, extname.substr(1), webpOptionStr)
          .then((webpBuffer) => {
            // Save to local
            if (webpSrcPath) {
                  ...
                  if (err) {
                    ...
                    console.log(
                      `[webp-loader] webp created success,compression ratio=${(
                        webpBuffer.length / content.length
                      ).toFixed(2)}%`
                    );
                    resolve(webpBuffer);
                  }
